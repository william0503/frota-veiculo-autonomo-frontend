...
export default function Profile() {
  ...
  useEffect(() => {
    api.get('rides/users/' + userPhone + '?page=' + page).then((res) => {
      setItems(res.data);
  ...
  useEffect(() => {
    api.get('rides/current/users/' + userPhone).then((res) => {
      setItem(res.data);
    });
  }, [userPhone, page]);
    ...
    if (item.length > 0) {
      if (item[0].status === 'asked') {
        return (
          <Link
            className="button"
            to={{ pathname: '/ride/start', state: item[0] }}
          >
            Ir para corrida
          </Link>
        );
        ...
        return (
          <Link
            className="button"
            to={{ pathname: '/ride/status', state: item[0] }}
          >
            Ir para corrida
          </Link>
        );
